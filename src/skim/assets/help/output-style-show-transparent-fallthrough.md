...
without obscuring the layer's own colour scheme.

When disabled, transparent keys are rendered blank — the previous skim
behaviour.

The ghost colour is the key's fill colour with its HSL lightness shifted
by 0.12 — lighter when the key sits at or below the layer's base colour
